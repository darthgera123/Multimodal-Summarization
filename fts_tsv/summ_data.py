...
# parser.add_argument("--topk", type=int, default=20, help='For testing only load topk feats from tsv')
# parser.add_argument("--num_features", type=int, default=36, help='How many features we have per img (e.g. 100, 80)')
...
class InputFeatures:
    ...
    def sent_features(self):
        return (self.input_ids,self.input_mask,self.segment_ids)
    def tit_features(self):
        return (self.decoder_input_ids,self.decoder_input_mask,self.decoder_segment_ids)

def preprocess_bert(sent,max_seq_len,tokenizer,tit=None,max_seq_len_title=None):
    ...
    sent,title = (input_ids,input_mask,segment_ids),(decoder_input_ids,decoder_input_mask,decoder_segment_ids)
    return sent,title

class HMTorchDataset(Dataset):
    def __init__(self, data_path,img_path,tsv_path,tr_name,max_seq_len,max_seq_len_title = None,train=True):
        super().__init__()
        # self.name = data_path
    ...
    def __getitem__(self, item: int):
        ...
        if self.train:
            sents,titles = preprocess_bert(sent,self.max_seq_len,self.tokenizer,title,self.max_seq_len_title)
            sent = torch.tensor([sents[0],sents[1],sents[2]])
            title = torch.tensor([titles[0],titles[1],titles[2]])
            return img_id, feats, boxes, sent,title
            # return img_id, feats, boxes, text, target
        else:
            sents,titles = preprocess_bert(sent,self.max_seq_len,self.tokenizer)
            sent = torch.tensor([sents[0],sents[1],sents[2]])
            return img_id, feats, boxes, sent
...
if __name__ == '__main__':
    test = HMTorchDataset('../data/dev_seen.jsonl','../data/img','../data/hm_vgattr3636.tsv')
    print(test.__len__())
